...
class RawFirBuilder(
    ...
    private val typeRefCache = IdentityHashMap<KtTypeReference, FirTypeRef>()
    ...
    private inner class Visitor : KtVisitor<FirElement, Unit>() {
        ...
        private fun KtTypeReference?.toFirOrErrorType(): FirTypeRef {
            ...
            typeRefCache[this]?.let { return it }
            return when (val firTypeRef = convertSafe<FirTypeRef>()) {
                null -> buildErrorTypeRef {
                    source = this@toFirOrErrorType.toFirSourceElement(FirFakeSourceElementKind.ErrorTypeRef)
                    diagnostic = ConeSimpleDiagnostic("Conversion failed", DiagnosticKind.Syntax)
                }
                else -> {
                    typeRefCache[this] = firTypeRef
                    firTypeRef
                }
            }
        }
            ...
            return buildBlock {
                ...
                statements += FirWhileLoopBuilder().apply {
                    ...
                    if (ktParameter != null) {
                        ...
                        val loopParameterSource =
                            expression.loopParameter?.toFirSourceElement(FirFakeSourceElementKind.DesugaredForLoop) ?: fakeSource
                        val firLoopParameter = generateTemporaryVariable(
                            ...
                            initializer = buildFunctionCall {
                                source = loopParameterSource
                                calleeReference = buildSimpleNamedReference {
                                    source = loopParameterSource
                                    name = Name.identifier("next")
                                }
                                explicitReceiver = generateResolvedAccessExpression(loopParameterSource, iteratorVal)
                            },
            ...
            return when {
                ...
                conventionCallName != null -> {
                    if (operationToken in OperatorConventions.INCREMENT_OPERATIONS) {
                        val cache = hashMapOf<PsiElement, FirExpression>()
                        val convertFunction: PsiElement.() -> FirExpression = {
                            cache.getOrPut(this) { (this as KtExpression).toFirExpression("Incorrect expression inside inc/dec") }
                        }
